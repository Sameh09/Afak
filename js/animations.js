...
/* ── 9. NAVBAR: top pill → side dock on scroll ────────────────── */
function initNavbarScroll() {
  const header = document.querySelector('.header');
  if (!header) return;

  const DOCK_AT   = 160;
  const DOCKED_W  = 58;   /* matches CSS .header.nav-docked .header-inner { width: 58px } */
  const MARGIN    = 24;   /* px gap from viewport edge */

  let docked  = false;
  let ticking = false;

  function setX(px) { header.style.setProperty('--nx', px + 'px'); }
  function setY(px) { header.style.setProperty('--nt', px + 'px'); }

  /* X when centered: offset from left:50% to visually center the pill */
  function centerX() { return -Math.round(header.offsetWidth / 2); }

  /* X when side-docked: position so right edge (RTL) or left edge (LTR)
     sits MARGIN px from the viewport edge. left:50% is the anchor.       */
  function sideX() {
    const vw = window.innerWidth;
    const isRtl = document.documentElement.dir === 'rtl';
    return isRtl
      ? Math.round(vw / 2 - MARGIN - DOCKED_W)   /* right side */
      : Math.round(-vw / 2 + MARGIN);             /* left  side */
  }

  /* ── Init: set px values immediately (no transition on first set) ── */
  ...
  function dockSide() {
    if (docked) return;
    docked = true;
    header.classList.add('nav-docked', 'scrolled');
    /* rAF so class (width:auto) has applied before we measure/position */
    requestAnimationFrame(() => {
      setX(sideX());
      setY(Math.round(window.innerHeight * 0.3));
    });
  }
  ...
  function undock() {
    if (!docked) return;
    docked = false;
    header.classList.remove('nav-docked', 'scrolled');
    setX(centerX());
    setY(17.6);
  }

  function reposition() {
    if (docked) { setX(sideX()); setY(Math.round(window.innerHeight * 0.3)); }
    else          setX(centerX());
  }
  ...
  window.addEventListener('scroll', () => {
    ...
    ticking = true;
    requestAnimationFrame(() => {
      if (window.scrollY > DOCK_AT) dockSide();
      else                           undock();
      ticking = false;
    });
  }, { passive: true });
